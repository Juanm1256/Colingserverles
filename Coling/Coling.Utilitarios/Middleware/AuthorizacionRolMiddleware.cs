...
{
    public class AuthorizacionRolMiddleware : IFunctionsWorkerMiddleware
    {
        ...
        {
            var rolesClaims = funcionesRolesAutorizados.TryGetValue(nombreFuncion, out string? rolesString) && rolesString.Split(',').Contains(rolesClaim) ? true: false;
            return rolesClaims;
        }

        public void SetFunctionAuthorizadas(Dictionary<string, string> metodosAutorizados)
        {
